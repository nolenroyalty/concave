     ...
     type AIButtonProps = React.ButtonHTMLAttributes<HTMLButtonElement>;
        ...
        export function AIButton({ children, className, ...props }: AIButtonProps) {
          return (
           <button
           className={cn(
            "relative bg-transparent p-[3px] border-none overflow-hidden rounded-lg bg-gradient-to-r from-plum-p4/90 via-red-r3/90 to-yellow-y3/90 cursor-pointer group",
            className,
           )}
           {...props}
          >
